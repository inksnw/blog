...
通过把容器内部的类型放到 `trait` 中作为**输出类型**，使用 “关联类型” 增加了代码 的可读性。这样的 `trait` 的定义语法如下：
...
在下面例子中，我们使用 [std::marker::PhantomData](https://rustwiki.org/zh-CN/std/marker/struct.PhantomData.html) 作为虚类型参数的类型，创建 包含不同数据类型的元组。
