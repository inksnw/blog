...
闭包本质上很灵活,能在没有类型标注的情况下运行.这使得捕获 (caputre) 能够灵活地适应用例,既可以 (move),又可借用 (borrow).团包可以通过以下方式捕获变量:

- 通过引用: `&T`
- 通过可变引用: `&mut T`
- 通过值: `T`

团包优先通过引用来捕获变量,并且仅在需要时使用其它方式.
...
虽然Rust无需类型说明 就能在大多数时候完成变量捕获,但在编写函数时,这种模糊写法是不允许的.当以闭包作为输入参数时,必须批出闭包的完整类型,它是通过使用以下`trait`中的一种来指定的.其受限程度按以下顺序递减:
...
当闭包被定义,编译器会隐式的创建一个匿名结构体,用以存储闭包捕获的变量,同时为这个未知类型的结构体实现函数功能,通过`Fn`,`FnMut`或者`FnOnce`三种`trait`中的一种.
...
除此之外,还必须使用`move`关键字,它表明所有的捕获都是通过值进行的. 这是必须的,因为在函数退出时,任何通过引用的捕获都会被丢弃,在闭包中留下无效的引用.
...
和所有其他类型相反，这个类型无法实例化，因为此类型可能具有的所有可能值的集合为空。 注意，它与 `()` 类型不同，后者只有一个可能的值。如下面例子，虽然返回值中没有信息，但此函数会照常返回。
